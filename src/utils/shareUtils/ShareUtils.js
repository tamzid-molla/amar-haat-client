...
export const getPhotoURL = async (photo) => {
  ...
  try {
    const res = await axios.post(`https://api.imgbb.com/1/upload?key=${import.meta.env.VITE_imgBB_APIKEY}`, formData);
...
export const SaveUserInDb = async (userData) => {
   axios.post(`${import.meta.env.VITE_baseURL}/users`, userData);
};
